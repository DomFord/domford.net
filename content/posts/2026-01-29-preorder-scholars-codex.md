+++
title = "SCHOLAR'S CODEX: Preorder Now!"
authors = ["Dom Ford"]
date = 2026-01-29T15:00:00Z
description = "A collection of essays on FromSoftware games."
draft = false

[taxonomies]
tags = ["book","edited collection","FromSoftware", "Demon's  Souls", "Dark Souls", "Bloodborne", "Sekiro", "Elden Ring"]


[extra]
show_copyright = true
show_comments = true
show_shares = true
keywords = "book,edited collection,FromSoftware,Demon’s Souls,Dark Souls,Bloodborne,Sekiro,Elden Ring"
title_html = "SCHOLAR’S CODEX: Preorder Now!"
+++
{{ resize_image(path="static/images/scholars-codex.webp", width=500, height=500, op="fit_width") }}

🛒 [Preorder from Tune & Fairweather](https://www.tuneandfairweather.com/products/scholars-codex)

📖 [Get a free digital sampler here: introduction and the *Demon's Souls* section](https://www.tuneandfairweather.com/products/scholar-s-codex-free-digital-sampler)
# *Scholar's Codex*: Preorders now live!
A collection of essays on the *Souls* games written by academics from at least three continents. Each chapter takes its starting point from an item in one of the Souls games to talk about existentialism, humanity, irony, disempowerment, time and more.

Me and my co-editors, [Stefano Gualeni](https://gua-le-ni.com/), [Nele Van de Mosselaer](https://www.tilburguniversity.edu/staff/n-vandemosselaer) and [Daniel Vella](https://www.um.edu.mt/profile/danielmvella), have been working on this for a while, and keeping it secret has been hard. But now it's out there and you can all share in the excitement. Congratulations and thanks to all contributors.

Usually, this project would have been another edited collection with Routledge, or a special issue. But we wanted to do something a bit different, and to look a bit more outwards. We think that FromSoftware fans are particularly hungry for analysis and interested in the ideas dealt with in the games.

We felt that there was no reason that these chapters couldn't be both short and engaging *and* genuine contributions to scholarship. And, with the amazing work of [Tune & Fairweather](https://www.tuneandfairweather.com/), [Nico Delort](https://www.nicolasdelort.com/) and [Mark Wynne](https://www.markwynnedesign.com/), a beautiful object, too. We hope you enjoy!

## Editors
- Dom Ford (University of Bergen)
- Stefano Gualeni (University of Malta)
- Nele Van de Mosselaer (Tilburg University)
- Daniel Vella (University of Malta)

## Contributors
- Magdalena Kozyra (SWPS University)
- Richard Woodward (Humboldt University of Berlin)
- Dom Ford (University of Bergen)
- Marco Caracciolo (Ghent University)
- Daniel Vella (University of Malta)
- Nele Van de Mosselaer (Tilburg University)
- Andrea Andiloro (Swinburne University of Technology)
- Matteo Lupetti (Freelance art and game critic)
- Agata Waszkiewicz (John Paul II Catholic University of Lublin)
- Stefano Gualeni (University of Malta)
- Sarah Zaidan (Emerson College)
- Kelsey Janse (Tilburg University)
- Stefano Caselli (Independent researcher)
- Charlene Putney (NEON AURELIUS)
- Michał Kozub(Independent researcher)
- Iga Ewa Smoleńska (Independent researcher)
- Angelo M. Andriano (IULM University in Milan)
- Ricardo Fassone (University of Turin)
- Mikael D. Sebag (University of California, Irvine)
- Florence Smith Nicolls (Queen Mary University of London)
- Jennifer J. T. Cooke (Newcastle University)

## Creative direction
- Jason Killingworth

## Illutrations
- [Nico Delort](https://www.nicolasdelort.com/)

## Design
- [Mark Wynne](https://www.markwynnedesign.com/)

## Copyediting and proofreading
- Christian Donlan
- Ian Anderson
