...
It’s easier to programmatically transform sentence case into title case than vice versa. Using the `Short Title` field, you can also make it correctly capitalise the first letter of a subtitle. Put the everything before the colon in the `Short Title` field. You can also use HTML tags like `<i>` `</i>` to italicise words. If you right click on the `Title`, you can select `Sentence case` to do this automatically, althought it doesn't always treat proper nouns with the respect they deserve. Input titles into your Zotero in *full sentence case* like this:
